...
const identityFields: IdentityField[] = [
  { id: "identityExternalId", label: "external_id", key: DFIdentity.EXTERNAL_ID, inputType: "text", defaultValue: () => "npm-mpa-" + state.runId },
  { id: "identityEmail", label: "email", key: DFIdentity.EMAIL, inputType: "email", defaultValue: () => "npm-mpa+" + state.runId + "@example.com" },
...
const profileFields: ProfileField[] = [
  { id: "profileName", label: "df_name", key: DFUserProfile.NAME, kind: "string", defaultValue: "NPM MPA" },
...
function serviceQuery(): string {
  return "?serviceId=" + encodeURIComponent(state.serviceId) + "&runId=" + encodeURIComponent(state.runId);
}
...
function renderDynamic(): void {
  ...
  const homeLink = byId("mpaHome") as HTMLAnchorElement | null;
  const productLink = byId("mpaProduct") as HTMLAnchorElement | null;
  const cartLink = byId("mpaCart") as HTMLAnchorElement | null;
...
export function mountMpaPage(route: RouteName): void {
